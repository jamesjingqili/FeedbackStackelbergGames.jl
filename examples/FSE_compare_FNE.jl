...
Q1 = tmp1 - 0.5*collision_avoidance_mat; # set to be 0.5, TODO!!!
Q2 = tmp2 - 0.5*collision_avoidance_mat;
# Q3 = Q1;
# Q4 = Q2;
# Q1 = Q3 + Q4;
# Q2 = Q3 + Q4;
...
marker_alpha_list = 1*[0.9^(horizon+1-t) for t in 1:horizon+1]
scatter([x_st[t][1] for t in 1:horizon+1], [x_st[t][2] for t in 1:horizon+1],color=:red,markershape=:square, label="", seriesalpha=marker_alpha_list)
scatter!([x_st[t][5] for t in 1:horizon+1], [x_st[t][6] for t in 1:horizon+1],color=:blue,markershape=:square, label="", seriesalpha=marker_alpha_list)
scatter!([x_ne[t][1] for t in 1:horizon+1], [x_ne[t][2] for t in 1:horizon+1],color=:red,markershape=:circ, label="", seriesalpha=marker_alpha_list)
scatter!([x_ne[t][5] for t in 1:horizon+1], [x_ne[t][6] for t in 1:horizon+1],color=:blue,markershape=:circ, label="", seriesalpha=marker_alpha_list)

scatter!([x_st[horizon+1][1]], [x_st[horizon+1][2]],color=:red,markershape=:square, label="player 1, FSE")
scatter!([x_st[horizon+1][5]], [x_st[horizon+1][6]],color=:blue,markershape=:square, label="player 2, FSE")
scatter!([x_ne[horizon+1][1]], [x_ne[horizon+1][2]],color=:red,markershape=:circ, label="player 1, FNE")
scatter!([x_ne[horizon+1][5]], [x_ne[horizon+1][6]],color=:blue,markershape=:circ, label="player 2, FNE")
plot!(size = (400,300),xlabel="x position",ylabel="y position",grid=false, left_margin=10mm, bottom_margin=10mm, right_margin=10mm,
legend = (0.7,0.9))
savefig("fbst_fbne.pdf")


eig_of_S_list = [eigvals(Array(S_list[t]))  for t in 1:horizon]
