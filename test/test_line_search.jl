...
lq_approximation!(lq_approx, nonlinear_g, optimal_op)

Mₜ_true, Nₜ_true, nₜ_true = constrained_fbst_lq_solver!(
    π, 
    lq_approx, 
    true,
    false
...
# Mₜ_tmp, Nₜ_tmp, nₜ_tmp = constrained_fbst_lq_solver!(
#     π, 
#     g, 
#     true,
#     false,
...
α_opt, new_loss_opt, next_op_opt = line_search!(
    π,
    Δ,
    optimal_op, 
    g,
...
# TODO: test whether line search with Newton method converges for unconstrained LQ games!

lq_approx = deepcopy(g);

current_op = trajectory(
    x = [[1.0; 2.0; 1.0; 3.0] for t in 1:horizon+1],
    u = [zeros(nu) for t in 1:horizon],
    λ = [zeros(nx*n_players) for t in 1:horizon],
    η = [zeros(nu) for t in 1:horizon-1],
    ψ = [zeros(m) for t in 1:horizon],
