...
g1 = [-1.8]*turn_off_inequality_constraints
g2 = [-1.8]*turn_off_inequality_constraints
Gx1_terminal = [0 1.0]*turn_off_inequality_constraints
Gx2_terminal = [0 1.0]*turn_off_inequality_constraints
g1_terminal = [-2.1]*turn_off_inequality_constraints
g2_terminal = [-2.1]*turn_off_inequality_constraints
...
initial_op = trajectory(
    ...
    s = [ones(inequality_constraints_size*n_players) for t in 1:horizon+1]
);
...
lq_approximation!(lq_approx, nonlinear_g, current_op);
x, u, s, γ, μ, λ, η, ψ, Mₜ, Nₜ, nₜ = nw_pdip_fbst_lq_solver!(π, lq_approx, current_op, 1/1.0, false, true, true);

# x, u, s, γ, μ, λ, η, ψ, Mₜ, Nₜ, nₜ = nw_pdip_fbst_lq_solver!(π, lq_approx, current_op, 1/1.0, false, false, true);
...
for outer_iter = 1:num_outer_iter
    ...
    for t = 1:horizon+1
        ...
        #     );
        #     calibrated_γ[t] = diagm(tmp)^(-1)*(ρ_list[outer_iter].*ones(2*inequality_constraints_size));
        #     calibrated_s[t] = tmp;
        # else
        #     tmp = vcat(
    ...
    for iter in 1:num_iter
        ...
        x, u, s, γ, μ, λ, η, ψ, Mₜ, Nₜ, nₜ = nw_pdip_fbst_lq_solver!(
            π, 
            lq_approx, 
            current_op, 
            ρ_list[outer_iter], 
