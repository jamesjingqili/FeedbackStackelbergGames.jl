...
h1 = [0]
h2 = [0]
Hx1_terminal = [0 0 0 1.0]*0.0
Hx2_terminal = [0 0 0 1.0]*0.0
h1_terminal = [-1.]*0.0
h2_terminal = [-1.]*0.0
...
g1 = [0]
g2 = [0]
Gx1_terminal = [0 0 0 1.0];#; 0 0 0 0]
Gx2_terminal = [0 0 0 1.0];#; 0 0 0 0]
g1_terminal = [-1.0];#; 0]
g2_terminal = [-1.0];#; 0]
...
f_list = [(x, u)-> A_list[t]*[x[1]; x[2]; x[3]; x[4]] + B_list[t]*u for t in 1:horizon];
costs_list = [
...
current_op = trajectory(
    ...
    γ = [ones(inequality_constraints_size*n_players) for t in 1:horizon+1],
)
...
x_true, u_true, γ_true, μ_true, λ_true, η_true, ψ_true, M_true, N_true, n_true = pdip_fbst_lq_solver!(
    π, 
    g, 
    current_op,
    1/1.0,
...
current_op = trajectory(
    ...
    ψ = [zeros(m) for t in 1:horizon],
    μ = [zeros(equality_constraints_size*n_players) for t in 1:horizon+1],
    γ = [0.1*ones(inequality_constraints_size*n_players) for t in 1:horizon+1],
);
next_op = deepcopy(current_op);
forward_simulation!(current_op, nonlinear_g);

lq_approximation!(lq_approx, nonlinear_g, current_op);
x, u, γ, μ, λ, η, ψ, Mₜ, Nₜ, nₜ = pdip_fbst_lq_solver!(π, lq_approx, current_op, 1/10.0, false, true);
...
# the line search test begins now:
...
for iter in 1:num_iter
    global x, u, γ, μ, λ, η, ψ, Mₜ, Nₜ, nₜ, Δ, α, new_loss, next_op, current_op
    lq_approximation!(lq_approx, nonlinear_g, current_op)
    x, u, γ, μ, λ, η, ψ, Mₜ, Nₜ, nₜ = pdip_fbst_lq_solver!(π, lq_approx, current_op, 1/1.0, false, false)
    Δ = trajectory(
        x = x,
        u = u,
        λ = λ,
    ...
    α, new_loss, next_op = pdip_line_search!(
        π,
        Δ,
        current_op, 
        lq_approx,
