...
h1 = [0]
h2 = [0]
Hx1_terminal = [1.0 0 0 0]*0.0
Hx2_terminal = [1.0 0 0 0]*0.0
h1_terminal = [-1.]*0.0
h2_terminal = [-1.]*0.0
...
g1 = [-1.8]*0.0
g2 = [-1.8]*0.0
Gx1_terminal = [0 0 0 1.0]*0.0;
Gx2_terminal = [0 0 0 1.0]*0.0;
g1_terminal = [-1.0]*0.0;
g2_terminal = [-1.0]*0.0;
...
homotopy_loss_list = [zeros(num_iter) for ii in 1:num_outer_iter];
...
for outer_iter = 1:num_outer_iter
    global x, u, γ, μ, λ, η, ψ, Mₜ, Nₜ, nₜ, Δ, α, ρ, new_loss, next_op, current_op, lq_approx, early_stop_counter
    if outer_iter == 1
        ρ_list[outer_iter] = ρ₀;
    else
    ...
    for iter in 1:num_iter
        # lq_approximation!(lq_approx, nonlinear_g, current_op) # update lq approximation? Be careful. pdip_line_search also calls this function.
        ...
        α, new_loss, next_op, lq_approx, min_ineq, min_γ, homotopy_loss = pdip_line_search(
            π,
            Δ,
            current_op, 
            lq_approx,
        ...
        homotopy_loss_list[outer_iter][iter] = homotopy_loss;
